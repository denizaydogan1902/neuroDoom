...
def capture() -> dict[str, np.ndarray]:
    ...
    out["start"] = frame(None)[0]
    out["advanced"] = frame((144, 112, 0))[0]       # ileri yürümüş
    out["turned"] = frame((112, 112, 16))[0]        # 90° dönülmüş
    out["retreat"] = frame((80, 112, 0))[0]         # odaya geri çekilmiş
    return out
